...
The data is arranged in rows and columns in a tabular data. Pandas Dataframe is a good package to deal with this format as it is designed to handle data which is arranged in a set of labeled columns and numbered by row indexes; the columns can have different data types, but elements in a single column must be the same datatype. In this post we will use the information that we extracted from the Getting Disclosed GeoInfo post from the Access Database from Victoria State Goverment. As a first example let's evaluate the `quantVal` column.
